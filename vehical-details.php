...
// Get the current maximum booking number from the database
$sql_max_bookingno = "SELECT MAX(BookingNumber) AS max_bookingno FROM tblbooking";
$query_max_bookingno = $dbh->prepare($sql_max_bookingno);
$query_max_bookingno->execute();
$result_max_bookingno = $query_max_bookingno->fetch(PDO::FETCH_ASSOC);
$max_bookingno = $result_max_bookingno['max_bookingno'];

// Increment the maximum booking number and format it with leading zeros
$bookingno = str_pad($max_bookingno + 1, 6, "0", STR_PAD_LEFT);
...
<?php }} ?>
  ...
  <input type="date" class="form-control" name="fromdate" placeholder="From Date" min="<?php echo date('Y-m-d'); ?>" required>
</div>
<div class="form-group">
  <label>To Date:</label>
  <input type="date" class="form-control" name="todate" placeholder="To Date" min="<?php echo date('Y-m-d'); ?>" required onblur="validateDate()">
  <span id="dateError" style="color: red; display: none;">To Date should be greater than From Date.</span>
</div>
